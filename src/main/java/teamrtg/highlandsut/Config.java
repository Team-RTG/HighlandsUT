...
public class Config
{
	...
	private static void addSettingsEntries(Configuration config) 
	{
		...
		biomePrefix.setComment("use a prefix of \"Highlands_\" + biome name for all biomes?");
		biomeSize = config.get(config.CATEGORY_GENERAL, "Biome Size", 4);
		biomeSize.setComment("Biome size in Highlands worlds - 4 is default (same as Default worldtype)");
		LBbiomeSize = config.get(config.CATEGORY_GENERAL, "Large Biomes Biome Size", 6);
		LBbiomeSize.setComment("Biome size in Highlands LB(Large Biome) worlds - 6 is default (same as Large Biomes worldtype)");
		//genDefault = config.get(config.CATEGORY_GENERAL, "Highands biomes in Default worlds", false);
		//genDefault.setComment("Should Highlands biomes generate in the Default and Large Biomes worldtype (for compatibilty with other biome and worldgen mods)");
