...
public class HLEventManager {
	...
	public void onDecorateTree(Decorate e)
	{
		if(e.type == Decorate.EventType.TREE){
			...
			Biome biome = e.world.getBiomeGenForCoords(e.pos);
			
			/*
			if(biome.equals(Biome.forest) && e.rand.nextInt(18) == 1){
			...
			*/
			...
			/*
			if(biome.equals(Biome.forest) && e.rand.nextInt(20) == 1){
				e.setResult(Event.Result.DENY);
				HighlandsGenerators.greatOakGen.generate(e.world, e.rand, e.pos);
			}
			if(biome.equals(Biome.forest) && e.rand.nextInt(7) == 1){
				e.setResult(Event.Result.DENY);
				new WorldGenBigTree(false).generate(e.world, e.rand, e.pos);
			}
			
			if(biome.equals(Biome.birchForest) && e.rand.nextInt(12) == 1){
				e.setResult(Event.Result.DENY);
				HighlandsGenerators.poplarGen.generate(e.world, e.rand, e.pos);
			}
			*/
			if(biome.equals(Biome.savanna) && e.rand.nextInt(3) != 1){
				e.setResult(Event.Result.DENY);
			}
		}
	}
